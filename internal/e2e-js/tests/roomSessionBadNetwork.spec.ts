...
test.describe('roomSessionBadNetwork', () => {
  ...
  test('should survive to a network switch for member', async ({
    createCustomPage,
  }) => {
    const page = await createCustomPage({
      name: '[bad-network-member]',
    })
    ...
    const connectionSettings = {
      vrt: {
        room_name: roomName,
        user_name: `e2e_bad_network_member`,
        join_as: 'member',
    ...
    const promise2 = await makeMemberUpdatedPromise()

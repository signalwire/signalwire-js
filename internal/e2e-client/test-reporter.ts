...
import { createContextDumpText } from './utils'
...
class TestNameReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    if (status === 'failed' && result.errors.length > 0) {
      ...
      result.errors.forEach((error: TestError, index: number) => {
        ...
        if (error.message?.includes('page.evaluate')) {
          ...
          if (error.message?.includes('Timeout')) {
            ...
            console.log(
              'HINT: One or more promises inside page.evaluate may have timed out'
            )
          }
          ...
          if (error.message?.includes('Object')) {
            ...
            console.log(
              'HINT: Ensure all returned values from page.evaluate are serializable'
            )
          }
        }
        ...
        if (
          ...
          const eventMatch = error.message.match(
            /waiting for (?:event|SDK event)[:\s]+['"]?([^'"]+)['"]?/i
          )
          if (eventMatch) {
      ...
      // Add SDK Test Context dump for failed tests (only if TestContext is available)
      const hasTestContext = result.attachments.some(
        (att) => att.name === 'SDK Test Context (JSON)'
      )
      if (hasTestContext) {
        this.dumpTestContext(result)
      }
    }
  ...
  private dumpTestContext(result: TestResult) {
    // Find SDK Test Context attachment (we know it exists from the caller check)
    const contextAttachment = result.attachments.find(
      (att) => att.name === 'SDK Test Context (JSON)'
    )!

    try {
      const contextData = JSON.parse(contextAttachment.body!.toString())
      console.log('\n[INFO] Using TestContext from attachment')

      // Use the existing utility to format and display the context dump
      const formattedDump = createContextDumpText(contextData)
      console.log('\n' + formattedDump)
    } catch (error) {
      console.log(
        '\n[WARNING] Failed to parse TestContext from attachment:',
        error
      )
    }
  }

...
export class DeviceRecoveryEngine extends EventEmitter<DeviceRecoveryEngineEvents> {
  ...
  private readonly recoveryStrategies = new Map<string, RecoveryStrategyDefinition>()
  
  /**
   * Debounced recovery functions per device type
   */
  private readonly debouncedRecovery = new Map<DeviceType, (
   ...
   */
  constructor(options?: DeviceRecoveryEngineOptions) {
    super()
    
    this.options = this.mergeOptions(options)
    
    if (this.options.debug) {
      this.logger.debug('[DeviceRecoveryEngine] Initialized with options:', this.options)
    }
  ...
  async recoverDevice(
    ...
    if (this.activeRecoveries.has(deviceType)) {
      ...
      this.logger.debug(`[DeviceRecoveryEngine] Recovery already active for ${deviceType}`, activeAttempt.id)
    ...
    if (recentAttempts.length >= this.options.maxRecoveryAttempts) {
      this.logger.warn(`[DeviceRecoveryEngine] Maximum recovery attempts exceeded for ${deviceType}`)
      
      return {
        success: false,
        error: new Error(`Maximum recovery attempts (${this.options.maxRecoveryAttempts}) exceeded`),
        method: 'custom',
    ...
    const attempt = this.createRecoveryAttempt(deviceType, currentState, preferences)
    ...
    try {
      ...
      if (result.success) {
        ...
        this.emit('recovery.failed', { attempt, error: result.error || new Error('Recovery failed') })
      }
      
      return result
      
    } catch (error) {
      const recoveryError = error instanceof Error ? error : new Error('Unknown recovery error')
      
      attempt.error = recoveryError
      attempt.endTime = Date.now()
      this.updateRecoveryStatus(attempt, RecoveryStatus.FAILED)
      
      this.emit('recovery.failed', { attempt, error: recoveryError })
  ...
  async tryStrategy(
    ...
    if (strategy.canHandle && !strategy.canHandle(deviceType, currentState, preferences, availableDevices)) {
      return {
        success: false,
        error: new Error(`Strategy '${strategyName}' cannot handle current situation`),
        reason: 'Strategy not applicable'
      }
    }

    try {
      const result = await strategy.execute(deviceType, currentState, preferences, availableDevices)
      
      if (this.options.debug) {
        this.logger.debug(`[DeviceRecoveryEngine] Strategy '${strategyName}' result:`, result)
      }
      
      return result
      
    } catch (error) {
      const strategyError = error instanceof Error ? error : new Error('Strategy execution failed')
  ...
  async getAvailableDevices(): Promise<MediaDeviceInfo[]> {
    try {
      ...
      this.logger.error('[DeviceRecoveryEngine] Failed to enumerate devices:', error)
  ...
  registerStrategy(strategy: RecoveryStrategyDefinition): void {
    ...
    if (this.options.debug) {
      this.logger.debug(`[DeviceRecoveryEngine] Registered strategy: ${strategy.name}`)
    }
  }
  ...
  unregisterStrategy(strategyName: string): void {
    ...
    if (this.options.debug) {
      this.logger.debug(`[DeviceRecoveryEngine] Unregistered strategy: ${strategyName}`)
    }
  }
  ...
  getRecoveryHistory(deviceType?: DeviceType): RecoveryAttempt[] {
    if (deviceType) {
      return this.recoveryHistory.filter(attempt => attempt.deviceType === deviceType)
    }
  ...
  getRecoveryStatus(): { activeRecoveries: number; queuedRecoveries: number } {
    const queuedCount = Array.from(this.recoveryQueue.values())
  ...
  cancelRecovery(deviceType: DeviceType, reason = 'Cancelled by user'): void {
    ...
    if (activeAttempt) {
      ...
      this.emit('recovery.cancelled', { attempt: activeAttempt, reason })
      this.emitStatusChange()
      
      if (this.options.debug) {
        this.logger.debug(`[DeviceRecoveryEngine] Cancelled recovery for ${deviceType}:`, reason)
      }
    }
  }
  ...
  clearHistory(deviceType?: DeviceType): void {
    if (deviceType) {
      const filteredHistory = this.recoveryHistory.filter(attempt => attempt.deviceType !== deviceType)
      this.recoveryHistory.splice(0, this.recoveryHistory.length, ...filteredHistory)
    } else {
      this.recoveryHistory.splice(0, this.recoveryHistory.length)
    }
    
    if (this.options.debug) {
      this.logger.debug(`[DeviceRecoveryEngine] Cleared recovery history${deviceType ? ` for ${deviceType}` : ''}`)
    }
  }
   ...
   */
  private mergeOptions(options?: DeviceRecoveryEngineOptions): Required<DeviceRecoveryEngineOptions> {
    return {
  ...
  private initializeDefaultStrategies(): void {
    ...
    this.registerStrategy({
      name: 'exact-id-match',
      priority: 1,
      description: 'Try to find a device with the exact same ID as previously used',
      execute: async (deviceType, currentState, _preferences, availableDevices) => {
    ...
    this.registerStrategy({
      name: 'label-match',
      priority: 2,
      description: 'Try to find a device with the same label as previously used',
      execute: async (deviceType, currentState, preferences, availableDevices) => {
        const targetLabel = currentState?.label || preferences.find(p => p.priority === 1)?.label
        ...
        const matchingDevices = availableDevices.filter(d => 
          d.kind === kind && d.label === targetLabel
        )
      ...
      canHandle: (_deviceType, currentState, preferences) => 
        !!(currentState?.label || preferences.find(p => p.priority === 1)?.label)
    })
    ...
    this.registerStrategy({
      ...
      priority: 3,
      description: 'Try to find any available device of the same type from preferences',
      execute: async (deviceType, _currentState, preferences, availableDevices) => {
        const kind = DeviceRecoveryEngine.getMediaDeviceKind(deviceType)
      ...
      canHandle: (deviceType, _currentState, _preferences, availableDevices) => {
        const kind = DeviceRecoveryEngine.getMediaDeviceKind(deviceType)
    ...
    this.registerStrategy({
      ...
      execute: async (deviceType, _currentState, _preferences, availableDevices) => {
        const kind = DeviceRecoveryEngine.getMediaDeviceKind(deviceType)
        
        // Look for default device (empty deviceId)
        const defaultDevice = availableDevices.find(d => 
  ...
  private initializeDebouncedRecovery(): void {
    ...
    deviceTypes.forEach(deviceType => {
      const debouncedFn = debounce(
        async (type: DeviceType, currentState?: DeviceState, preferences?: DevicePreference[]) => {
          try {
            await this.recoverDevice(type, currentState, preferences)
          } catch (error) {
            this.logger.error(`[DeviceRecoveryEngine] Debounced recovery failed for ${type}:`, error)
          }
        },
    ...
    for (const strategy of strategies) {
      ...
      try {
        ...
        if (result.success && result.deviceId) {
          ...
          const isAvailable = await this.options.isDeviceAvailable(result.deviceId, deviceType)
          ...
          if (isAvailable) {
            ...
            if (this.options.debug) {
              this.logger.debug(`[DeviceRecoveryEngine] Device ${result.deviceId} not available, trying next strategy`)
            }
          }
        }
      } catch (error) {
        if (this.options.debug) {
          this.logger.debug(`[DeviceRecoveryEngine] Strategy '${strategy.name}' failed:`, error)
        }
        ...
        this.emit('strategy.executed', {
          ...
          result: {
            success: false,
            error: error instanceof Error ? error : new Error('Strategy execution failed')
          }
        })
      }
    }
   ...
   */
  private updateRecoveryStatus(attempt: RecoveryAttempt, status: RecoveryStatus): void {
    attempt.status = status
    
    if (this.options.trackRecoveryStatus) {
      if (this.options.debug) {
        this.logger.debug(`[DeviceRecoveryEngine] Recovery ${attempt.id} status: ${status}`)
      }
    }
  }
  ...
  private addToHistory(attempt: RecoveryAttempt): void {
    ...
    if (this.recoveryHistory.length > this.options.maxHistorySize) {
      this.recoveryHistory.splice(0, this.recoveryHistory.length - this.options.maxHistorySize)
    }
  }
  ...
  private getRecentAttempts(deviceType: DeviceType): RecoveryAttempt[] {
    ...
    return this.recoveryHistory.filter(attempt => 
      ...
      (now - attempt.startTime) < timeLimit
    )
  }
   ...
   */
  private getRecoveryMethod(strategyName: string): 'preference' | 'fallback' | 'any' | 'custom' {
    switch (strategyName) {
      case 'exact-id-match':
      case 'label-match':
        return 'preference'

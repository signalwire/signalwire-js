...
export const SignalWire = (() => {
  ...
  return (params: SignalWireClientParams): Promise<SignalWireClient> => {
    ...
    if (!instance) {
      instance = createSignalWireClient(params).then((client) => {
 ...
 */
export async function createSignalWireClient(params: SignalWireClientParams): Promise<SignalWireClient> {
  const { clientId = 'default', storage, ...restParams } = params
  
  // Create storage wrapper if storage is provided
  const wrappedStorage = storage ? new StorageWrapper(storage, clientId) : undefined

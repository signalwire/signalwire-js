...
import { addOverlayPrefix, SDK_PREFIX } from './utils/callSession'

describe('buildVideoElement', () => {
  describe('with CallSession', () => {
    let callSessionInstance: CallSession
    ...
    const setupRoomForTests = () => {
      // @ts-expect-error
      callSessionInstance.getRTCPeerById = jest.fn((_id: string) => mockPeer)
      // @ts-expect-error
      callSessionInstance.runRTCPeerWorkers(callId)
    }

    beforeEach(() => {
      stack = configureFullStack()
      store = stack.store
      callSessionInstance = createCallSessionObject({
        store,
      })
      setupRoomForTests()
    ...
    it('should take a video element and return it with unsubscribe function', async () => {
      ...
      const result = await buildVideoElement({
        callSessionInstance,
    ...
    it('should create a video element and return it with unsubscribe function', async () => {
      const result = await buildVideoElement({ callSessionInstance })
    ...
    it('should unsubscribe from all events on unsubscribe', async () => {
      ...
      const result = await buildVideoElement({
        callSessionInstance,
        rootElement: mockVideoEl,
      })

      // Mock the room.off function to spy on it
      callSessionInstance.off = jest.fn()

      result.unsubscribe()

      expect(callSessionInstance.off).toHaveBeenCalledWith(
        'track',
        expect.any(Function)
      )
      expect(callSessionInstance.off).toHaveBeenCalledWith(
    ...
    describe('with remoteVideoTrack', () => {
      ...
      const renderAndStartVideo = (
        params?: Omit<BuildVideoElementParams, 'callSessionInstance'>
      ) => {
      ...
      beforeEach(() => {
        ...
        // @ts-expect-error
        callSessionInstance.getRTCPeerById = jest.fn(
          (_id: string) => newPeerMock
        )
        // @ts-expect-error
        callSessionInstance.localStream = new MediaStream([])
      })
      ...
      it('should make video element that contains a remote video track', async () => {
        ...
        expect(mediaStream.getTracks()).toContain(
          // @ts-expect-error
          callSessionInstance.peer.remoteVideoTrack
        )
      ...
      it('should mirror the video overlay', async () => {
        ...
        expect(callSessionInstance.localVideoOverlay!.mirrored).toBe(true)
        expect(videoElement!.style.transform).toBe('scale(-1, 1)')

        callSessionInstance.localVideoOverlay!.setMirror(false)
        expect(callSessionInstance.localVideoOverlay!.mirrored).toBe(false)
        expect(videoElement!.style.transform).toBe('scale(1, 1)')

        callSessionInstance.localVideoOverlay!.setMirror(true)
        expect(callSessionInstance.localVideoOverlay!.mirrored).toBe(true)
        expect(videoElement!.style.transform).toBe('scale(-1, 1)')
      })
      ...
      it('should hide the video overlay', async () => {
        ...
        expect(callSessionInstance.localVideoOverlay!.status).toBe('hidden')

        callSessionInstance.localVideoOverlay!.show()
        expect(callSessionInstance.localVideoOverlay!.status).toBe('visible')

        callSessionInstance.localVideoOverlay!.hide()
        expect(callSessionInstance.localVideoOverlay!.status).toBe('hidden')
      })
    })
  })

...
export class WebRTCStatsMonitor implements IStatsMonitor {
  ...
  /**
   * Use a monitoring preset configuration (alias for applyPreset)
   */
  usePreset(preset: 'strict' | 'balanced' | 'relaxed'): void {
    this.applyPreset(preset as keyof typeof MONITORING_PRESETS)
  }

  /**
   * Set monitoring thresholds
   */
  setThresholds(thresholds: Partial<MonitoringOptions['thresholds']>): void {
    this.updateOptions({ thresholds })
  }
  ...
  /**
   * Destroy the monitor and clean up resources (alias for dispose)
   */
  destroy(): void {
    this.dispose()
  }

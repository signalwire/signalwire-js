...
export const sessionConnectionPoolWorker: SDKWorker<SessionConnectionPoolWorkerOptions> = function* (
  options
): SagaIterator {
  ...
  try {
    ...
    if (!iceServers || iceServers.length === 0) {
      logger.warn('No ICE servers available, cannot initialize connection pool')
      return
    }
    ...
    yield sagaEffects.call(
      [connectionPoolManager, connectionPoolManager.cleanup]
    )

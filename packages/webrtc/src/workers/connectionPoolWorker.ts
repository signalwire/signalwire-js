...
export const connectionPoolWorker: SDKWorker<ConnectionPoolWorkerOptions> =
  function* (options): SagaIterator {
    ...
    try {
      ...
      if (!iceServers || iceServers.length === 0) {
        logger.debug(
          'No ICE servers available yet, waiting for session.connected'
        )
      ...
      const config: RTCConfiguration = {
        ...
        iceCandidatePoolSize:
          (instance as any).options?.iceCandidatePoolSize || 10,

/**
 * RTCPeerCore - A decoupled version of RTCPeer without direct @signalwire/core dependencies
 * 
 * This implementation uses dependency injection to remove direct coupling to @signalwire/core
 * while preserving the exact same logic and behavior as the original RTCPeer.
 */
...
// Minimal logger interface needed by RTCPeerCore
export interface RTCPeerLogger {
  debug(...args: any[]): void
  info(...args: any[]): void
  warn(...args: any[]): void
  error(...args: any[]): void
...
// Minimal call contract interface that RTCPeerCore needs from BaseConnection
export interface RTCPeerCallContract<_EventTypes = any> {
  // Properties
  id: string
  options: any
  ...
  onLocalSDPReady(peer: any): Promise<void>
...
export interface RTCPeerDependencies {
  logger: RTCPeerLogger
  uuidGenerator: () => string
}

export default class RTCPeerCore<_EventTypes = any> {
  public uuid: string
  public call: RTCPeerCallContract<_EventTypes>
  public type: RTCSdpType
  public instance: RTCPeerConnection

  private logger: RTCPeerLogger
  ...
  constructor(
    call: RTCPeerCallContract<_EventTypes>, 
    type: RTCSdpType,
    dependencies: RTCPeerDependencies
  ) {
    // Inject dependencies
  ...
  startWatchMediaPackets() {
    ...
    this._mediaWatcher = watchRTCPeerMediaPackets(this as any)

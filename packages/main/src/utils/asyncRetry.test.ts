...
describe('asyncRetry', () => {
  ...
  describe('retries', () => {
    ...
    it('should call the promise without delay function and maximum retries', async () => {
      const delaySpy = vi.fn(() => delayFn());
      const callableSpy = vi.fn(() => Promise.resolve());
    ...
    describe('With fake timers', () => {
      ...
      test('should retry and succeed on second attempt', async () => {
        const delaySpy = vi.fn(() => constDelayFn());
        const mockAsyncCallable = vi
          .fn()
          .mockRejectedValueOnce(new Error('Failed'))
          .mockResolvedValue('success');
        ...
        // Advance timer
        vi.advanceTimersByTime(CONST_DELAY_INTERVAL / 2);
      ...
      it('Should throw a validator exception transparently', async () => {
        const delaySpy = vi.fn(() => constDelayFn());
        const callableSpy = vi.fn(() => Promise.resolve({ ok: false }));
        const validatorSpy = vi.fn((result: { ok: boolean }) => {
          if (!result.ok) {
      ...
      it('Should throw the real exception if fail the last attempt', async () => {
        ...
        try {
          ...
          expect((error as Error).message).toBe('Real Callable Error - should be thrown');
          await expect(() => promise).rejects.toEqual(
            new Error('Real Callable Error - should be thrown')
          );
        }
      });
    });
    ...
    it('Should not throw a validator exception', async () => {
      const delaySpy = vi.fn(() => delayFn());
      const callableSpy = vi.fn(() => Promise.resolve({ ok: true }));

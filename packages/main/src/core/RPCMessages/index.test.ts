...
describe('RPC Messages', () => {
  describe('RPCConnect', () => {
    ...
    it('should include dpop_token when provided', function () {
      const authentication = { jwt_token: 'jwt' };
      const message = RPCConnect({
        authentication,
        dpop_token: 'dpop-proof-jwt'
      });
      expect(message).toStrictEqual({
        jsonrpc: '2.0',
        id: 'mocked-uuid',
    ...
    it('should omit dpop_token when not provided', function () {
      const authentication = { jwt_token: 'jwt' };
      const message = RPCConnect({ authentication });
      expect(message.params).not.toHaveProperty('dpop_token');
    });
  });
  ...
  describe('RPCReauthenticate', () => {
    ...
    it('should include dpop_token as sibling of authentication when provided', function () {
      const message = RPCReauthenticate({
        project: 'project',
        jwt_token: 'jwt',
        dpop_token: 'dpop-proof-jwt'
    ...
    it('should omit dpop_token when not provided', function () {
      ...
      expect(message.params).not.toHaveProperty('dpop_token');
      expect(message.params.authentication).not.toHaveProperty('dpop_token');
    });
  });

  describe('RPCPing', () => {
    it('should generate the message', function () {
      vi.spyOn(global.Date, 'now').mockReturnValue(1581442824134);
  ...
  describe('Verto RPC Messages', () => {
    ...
    const allVertoMethods = [VertoBye, VertoAttach, VertoInfo, VertoInvite, VertoModify];

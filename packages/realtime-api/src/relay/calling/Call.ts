...
import {
  prepareRecordParams,
  preparePlayParams,
  preparePlayAudioParams,
  preparePromptParams,
  preparePromptAudioParams,
  preparePromptTTSParams,
  prepareTapParams,
  preparePromptRingtoneParams,
  prepareConnectParams,
} from '../helpers'
...
export class Call implements ICall {
  ...
  async _execute(msg: Execute) {
    try {
      ...
      logger.error(
        `Relay command failed with code: ${error.code} - ${error.message}`
      )
      throw error
    }
  }
  ...
  async disconnect() {
    ...
    this._addComponent(component)
    await component._waitFor(
      CallConnectState.Failed,
      CallConnectState.Disconnected
    )
  ...
  async play(
    ...params: [ICallingPlayParams] | (IRelayCallingPlay | ICallingPlay)[]
  ): Promise<PlayResult> {
  ...
  playAudio(
    params: string | { url: string; volume?: number }
  ): Promise<PlayResult> {
  ...
  async prompt(
    params: ICallingCollect,
    ...mediaList: (IRelayCallingPlay | ICallingPlay)[]
  ): Promise<PromptResult> {
    const [collect, play, volume] = preparePromptParams(params, mediaList)
    const component = new Prompt(this, collect, play, volume)
    this._addComponent(component)
    await component._waitFor(
      CallPromptState.Error,
      CallPromptState.NoInput,
      CallPromptState.NoMatch,
      CallPromptState.Digit,
      CallPromptState.Speech
    )
  ...
  promptTTS(
    params: ICallingCollectTTS,
    ttsOptions: ICallingPlayTTS = { text: '' }
  ): Promise<PromptResult> {
  ...
  async connect(
    ...params: [ICallingConnectParams] | DeepArray<IMakeCallParams>
  ...
  async detect(options: ICallingDetect): Promise<DetectResult> {
    ...
    this._addComponent(component)
    await component._waitFor(
      CallDetectState.Machine,
      CallDetectState.Human,
      CallDetectState.Unknown,
      CallDetectState.CED,
      CallDetectState.CNG
    )
  ...
  async detectHuman(params: ICallingDetect = {}): Promise<DetectResult> {
    logger.warn(
      'detectHuman has been deprecated: use detectAnsweringMachine instead.'
    )
    params.type = CallDetectType.Machine
    const result = await this.detect(params)
    result.component.successful =
      result.component.result === CallDetectState.Human
    return result
  }
  ...
  detectHumanAsync(params: ICallingDetect = {}): Promise<DetectAction> {
    logger.warn(
      'detectHumanAsync has been deprecated: use detectAnsweringMachineAsync instead.'
    )
    params.type = CallDetectType.Machine
    return this.detectAsync(params)
  }
  ...
  async detectMachine(params: ICallingDetect = {}): Promise<DetectResult> {
    logger.warn(
      'detectMachine has been deprecated: use detectAnsweringMachine instead.'
    )
    params.type = CallDetectType.Machine
    const result = await this.detect(params)
    result.component.successful =
      result.component.result === CallDetectState.Machine
    return result
  }
  ...
  detectMachineAsync(params: ICallingDetect = {}): Promise<DetectAction> {
    logger.warn(
      'detectMachineAsync has been deprecated: use detectAnsweringMachineAsync instead.'
    )
    params.type = CallDetectType.Machine
    return this.detectAsync(params)
  }
  ...
  detectFaxAsync({
    tone,
    timeout,
  }: ICallingDetect = {}): Promise<DetectAction> {
  ...
  async tap(
    params: ICallingTapTap | ICallingTapFlat,
    deprecatedDevice: ICallingTapDevice = {}
  ): Promise<TapResult> {
